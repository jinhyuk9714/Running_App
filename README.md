...
**Bucket4j** 기반 Token Bucket 알고리즘으로 API 요청 제한 구현

**엔드포인트별 차등 제한**

| 엔드포인트 | 제한 | 용도 |
|-----------|------|------|
| `POST /api/auth/login` | **10회/분** | 브루트포스 공격 방지 |
| `POST /api/auth/signup` | **5회/시간** | 스팸 계정 생성 방지 |
| 기타 API | **100회/분** | 일반 사용 |

**응답 헤더**

```
X-Rate-Limit-Remaining: 95      # 남은 요청 수
X-Rate-Limit-Retry-After-Seconds: 30  # 재시도까지 대기 시간 (제한 초과 시)
```
